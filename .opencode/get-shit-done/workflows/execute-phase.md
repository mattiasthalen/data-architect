...
VERIFIER_MODEL=$(node ./.opencode/get-shit-done/bin/gsd-tools.js resolve-model gsd-verifier --raw)
```
...
```bash
GSD_CONFIG=$(node ./.opencode/get-shit-done/bin/gsd-tools.js state load --raw)
...
MILESTONE_SLUG=$(node ./.opencode/get-shit-done/bin/gsd-tools.js generate-slug "$MILESTONE_NAME" --raw)
...
```bash
PHASE_INFO=$(node ./.opencode/get-shit-done/bin/gsd-tools.js find-phase "${PHASE_ARG}")
