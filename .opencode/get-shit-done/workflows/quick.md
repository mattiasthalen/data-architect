...
EXECUTOR_MODEL=$(node ./.opencode/get-shit-done/bin/gsd-tools.js resolve-model gsd-executor --raw)
```
...
```bash
ROADMAP_EXISTS=$(node ./.opencode/get-shit-done/bin/gsd-tools.js verify-path-exists .planning/ROADMAP.md --raw)
...
```bash
slug=$(node ./.opencode/get-shit-done/bin/gsd-tools.js generate-slug "$DESCRIPTION" --raw | cut -c1-40)
```
...
```markdown
| ${next_num} | ${DESCRIPTION} | $(node ./.opencode/get-shit-done/bin/gsd-tools.js current-timestamp date --raw) | ${commit_hash} | [${next_num}-${slug}](./quick/${next_num}-${slug}/) |
```
...
```
Last activity: $(node ./.opencode/get-shit-done/bin/gsd-tools.js current-timestamp date --raw) - Completed quick task ${next_num}: ${DESCRIPTION}
```
