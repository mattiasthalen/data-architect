...
```bash
PLANNING_EXISTS=$(node ./.opencode/get-shit-done/bin/gsd-tools.js verify-path-exists .planning --raw)
...
PLANNER_MODEL=$(node ./.opencode/get-shit-done/bin/gsd-tools.js resolve-model gsd-planner --raw)
CHECKER_MODEL=$(node ./.opencode/get-shit-done/bin/gsd-tools.js resolve-model gsd-plan-checker --raw)
```
...
PHASE_SLUG=$(node ./.opencode/get-shit-done/bin/gsd-tools.js generate-slug "$PHASE_NAME" --raw)
PADDED=$(node ./.opencode/get-shit-done/bin/gsd-tools.js find-phase "$PHASE" --raw | grep -o '^[0-9]*' || printf "%02d" "$PHASE")
...
```bash
WORKFLOW_RESEARCH=$(node ./.opencode/get-shit-done/bin/gsd-tools.js state load --raw | grep '^research=' | cut -d= -f2)
```
...
- **`## PLANNING COMPLETE`:** Display plan count. If `--skip-verify` or config `workflow.plan_check=false`: skip to step 13. Otherwise: step 10.
  ```bash
  WORKFLOW_PLAN_CHECK=$(node ./.opencode/get-shit-done/bin/gsd-tools.js state load --raw | grep '^plan_checker=' | cut -d= -f2)
  ```
