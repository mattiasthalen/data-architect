...
- Staging table reference uses a parameter placeholder pattern (e.g., `{staging_source}`) that Phase 8 will resolve with actual staging table names. For now, use a convention: `stg_{anchor_table_name}` as default staging source.
...
- Source: `stg_{anchor_table_name}` (default staging source convention)
- Key: `{mnemonic}_ID`
...
- Source: `stg_{anchor_table_name}` (attributes come from anchor's staging source)
