...
- `test_build_anchor_merge_uses_staging_table_name_convention`: Given an anchor, verify the staging source table name in `build_anchor_merge()` output matches `staging_table_name(anchor)` exactly. This validates DDL/DML naming consistency -- the same `naming.py` function used by DDL to create staging tables is used by DML to reference them.
