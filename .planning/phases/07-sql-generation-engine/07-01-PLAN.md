...
must_haves:
  truths:
    ...
    - "build_staging_table() produces CREATE TABLE IF NOT EXISTS from staging mapping definitions (table name + columns), and generate_all_ddl includes staging DDL when anchor.staging_mappings is populated (GEN-10)"
...
**Scope note:** This plan covers 5 entity types in 2 TDD tasks (~55% estimated context). The entity types share columns.py and naming.py foundations, making them tightly coupled -- splitting into two plans would create file ownership conflicts on the shared modules and break the RED-GREEN rhythm. Each individual builder function is small (10-30 lines of AST construction). If context pressure emerges during execution, the executor should prioritize anchors+attributes (most critical for 07-02 DML) and defer ties+staging to a gap closure plan.
</objective>
...
- `test_staging_table_name`: `staging_table_name({"table": "stg_customers", "columns": [...]})` returns `"stg_customers"` (passes through the table name from the mapping definition).
...
- `test_build_staging_table_creates_table`: Given a staging definition with name and column list (e.g., `{"table": "stg_customers", "columns": [{"name": "customer_id", "type": "bigint"}, {"name": "customer_name", "type": "varchar(100)"}]}`), verify `build_staging_table()` generates CREATE TABLE IF NOT EXISTS with those columns plus metadata columns.
- `test_build_staging_table_multi_dialect`: Verify staging DDL compiles to postgres, tsql, snowflake without errors.
- `test_generate_all_ddl_includes_staging_from_mappings`: Given a Spec where an anchor has `staging_mappings` populated (at least one mapping with table name and columns), verify `generate_all_ddl` includes the staging table DDL in the output dict. This validates GEN-10.
- `test_generate_all_ddl_skips_staging_when_no_mappings`: Given a Spec where no anchors have staging_mappings, verify `generate_all_ddl` output contains no staging table entries.
...
- CREATE TABLE IF NOT EXISTS with provided column name/type pairs
- Metadata columns (always)
- Columns are ordered: user-defined columns first, then metadata columns
- GEN-10: This generates staging table DDL from whatever column definitions exist in anchor.staging_mappings. Phase 8 will define the staging_mapping Pydantic models and column-level mapping semantics, but the DDL generation capability is established here.
...
- Iterate in deterministic order (GEN-08):
  ...
  4. Staging tables (GEN-10): For each anchor with non-empty `staging_mappings`, extract table name and column definitions from each mapping, call `build_staging_table()`, and include in output. Sort staging tables by name. If no anchors have staging_mappings, no staging tables are generated (no error, just empty).
...
- **Staging mapping structure assumption:** Each item in `anchor.staging_mappings` is a dict with `"table"` (str) and `"columns"` (list of `{"name": str, "type": str}` dicts). Phase 8 will formalize this into a proper Pydantic model; 07-01 reads from the raw dict structure.
