...
- Instructions for Data Architect to:
  ...
  6. When ready, delegate to @business-analyst for domain perspective and @system-analyst for source analysis
...
- Instructions for Data Architect to:
  ...
  4. Delegate to @veteran-reviewer for anti-pattern detection
  5. Summarize findings in structured format: Naming Issues, Structural Issues, Anti-Patterns, Recommendations
- Include note: "You orchestrate the review -- @veteran-reviewer provides critique, you synthesize."
...
- No skill mentions @business-analyst, @system-analyst, or other subagents in its own instructions -- skills tell the Data Architect what to do, and the Data Architect decides which subagents to involve
- Exception: /da:start can mention that the Data Architect should "delegate to @business-analyst and @system-analyst" since this is describing what the Data Architect does (the skill is talking TO the Data Architect)
- Keep skill instructions concise: 20-50 lines each
  </action>
  <verify>
Run `python -c "from data_architect.templates import TEMPLATES; skills = [k for k in TEMPLATES if '/skills/' in k]; print(f'{len(skills)} skills'); [print(k) for k in skills]"` to confirm 4 skill entries exist. Then verify all skill content contains `agent: data-architect` in frontmatter.
  </verify>
  <done>
TEMPLATES has 4 skill entries: da-start, da-review, da-status, da-export. All skills have `agent: data-architect` in YAML frontmatter. No skill directly invokes subagents. /da:export references `.data-architect/specs/` as export location.
  </done>
</task>
...
3. No skill SKILL.md directly invokes a subagent (no @business-analyst or @system-analyst in skill body, except through Data Architect instructions)
